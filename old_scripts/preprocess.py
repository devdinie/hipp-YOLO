...
def get_roi(msk_file, reserve=2):
        ...
        return [lower_idx, upper_idx] , [center_idx, normalized_whd]
...
def preprocess_data(data_dir):
        ...
        if settings.yolo_localize:
                os.mkdir(os.path.join(settings.data_dir,settings.augdata_dname,"annotations"))
        ...
        for idx in range(0,len(files_list)):
                ...
                if not(args.mode == "test") or settings.labels_available:
                        ...
                        [bb_Lbot, bb_Lup],[bb_Lctr, bb_Lwhd] = get_roi(msk_file_L, reserve=4)
                        [bb_Rbot, bb_Rup],[bb_Rctr, bb_Rwhd] = get_roi(msk_file_R, reserve=4)
                        #endregion split and flip labels only and get roi
                        
                        #region create annotations for yolo 
                        if settings.yolo_localize:
                                
                                annotL_fname = os.path.join(settings.data_dir,settings.augdata_dname,"annotations",
                                                           os.path.basename(img_fname).replace("_t1_","_t1_L_").replace(".nii",".txt"))
                                annotL_file = open(annotL_fname, 'a')
                                annotL_str = np.array2string(np.array([int(1),bb_Lctr[0],bb_Lctr[1],bb_Lctr[2],bb_Lwhd[0], bb_Lwhd[1], bb_Lwhd[2]]), separator=' ').strip("[]")
                                annotL_file.write(annotL_str+ "\n")
                                
                                annotR_fname = os.path.join(settings.data_dir,settings.augdata_dname,"annotations",
                                                           os.path.basename(img_fname).replace("_t1_","_t1_R_").replace(".nii",".txt"))
                                annotR_file = open(annotR_fname, 'a')
                                annotR_str = np.array2string(np.array([int(1),bb_Rctr[0],bb_Rctr[1],bb_Rctr[2],bb_Rwhd[0], bb_Rwhd[1], bb_Rwhd[2]]), separator=' ').replace("'","").strip("[]")
                                annotR_file.write(annotR_str+ "\n")
