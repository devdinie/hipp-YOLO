...
root_dir  = "/data/chamal/projects/swapna/YOLO_22v1/YOLO"
input_dir = "/data/chamal/projects/swapna/YOLO_22v1/data/data_input/yolo_inputs/yolo_model/"

def _main():
    annotation_path = os.path.join(input_dir,"yolo_inputs","data_train.txt")
    log_dir = "logs"
    if not os.path.exists(os.path.join(root_dir,log_dir)):
        os.mkdir(os.path.join(root_dir,log_dir))
        
    classes_path = os.path.join(input_dir,"yolo_inputs","data_classes.txt")
    # anchors_path = 'model_data/yolo-tiny_anchors.txt'
    anchors_path = os.path.join(root_dir,"keras_yolo3","model_data","yolo_anchors.txt")
    
    class_names = get_classes(classes_path)
    num_classes = len(class_names)
    anchors = get_anchors(anchors_path)

    input_shape = (144,144,144)  # multiple of 32, hw
    epoch1, epoch2 = 40, 40
    ...
    model = create_model(
        ...
        num_classes,
        freeze_body=2,
        weights_path=os.path.join(root_dir,"keras_yolo3","model_data","yolo.h5"))
