...
def create_yolo_inputs(annot_list):
        ...
        with open(os.path.join(yolo_inputdir,'data_train.csv'), 'a') as file:
                for line in train_list: file.write(line+'\n')
                
        with open(os.path.join(yolo_inputdir,'data_test.csv'), 'a') as file:
                for line in test_list: file.write(line+'\n')
        
        #region create yolo input lists
        
        endtime_train = time.time()
        print("- Network {}: creating input lists for yolo complete.".format(2, endtime_train-starttime_train))
